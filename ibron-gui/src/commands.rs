...
pub fn derive_command_from_key_assignment(action: &KeyAssignment) -> Option<CommandDef> {
    Some(match action {
        ...
        BlockRerun => CommandDef {
            brief: "Rerun focused block".into(),
            doc: "Re-runs the command of the focused block (not yet implemented)".into(),
            keys: vec![],
            args: &[ArgType::ActivePane],
            menubar: &["Shell", "Blocks"],
            icon: None,
        },
        BlockFold => CommandDef {
            brief: "Toggle fold on focused block".into(),
            doc: "Toggles a visual 'folded' mark on the focused block; the gutter strip dims to indicate the fold".into(),
            keys: vec![(Modifiers::CTRL.union(Modifiers::SHIFT), "f".into())],
            args: &[ArgType::ActivePane],
            menubar: &["View", "Blocks"],
            icon: None,
        },
        BlockBookmark => CommandDef {
            brief: "Toggle bookmark on focused block".into(),
            doc: "Toggles a bookmark on the focused block; bookmarked blocks get an extra gutter outrigger".into(),
            keys: vec![(Modifiers::CTRL.union(Modifiers::SHIFT), "b".into())],
            args: &[ArgType::ActivePane],
            menubar: &["View", "Blocks"],
            icon: None,
        },
        BlockShare => CommandDef {
            brief: "Share focused block as markdown".into(),
            doc: "Copies the focused block to the clipboard as markdown with fenced command and output sections".into(),
            keys: vec![],
            args: &[ArgType::ActivePane],
            menubar: &["Shell", "Blocks"],
            icon: None,
        },
        BlockAskAI => CommandDef {
            brief: "Copy focused block as an AI prompt".into(),
            doc: "Copies the focused block as markdown prefixed with an 'explain or debug' prompt, ready to paste into an AI assistant".into(),
            keys: vec![],
            args: &[ArgType::ActivePane],
            menubar: &["Shell", "Blocks"],
            icon: None,
        },
        BlockSearchOpen => CommandDef {
            brief: "Copy list of all command blocks".into(),
            doc: "Copies a markdown list of every command block in the pane with its status and first-line command".into(),
            keys: vec![],
            args: &[ArgType::ActivePane],
            menubar: &["View", "Blocks"],
            icon: None,
